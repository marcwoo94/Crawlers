...
driver.find_element_by_name("SMemberID").send_keys("eagle930")
driver.find_element_by_name("SMemberPassword").send_keys("%eagle4622")
...
def save_in_excel():
    ...
    column_title_2 = ["번호", "주제명", "서명", "저자", "발행자", "발행년", "책 수", "단가", "구입가격"]
    ...
    for row_index in range(1, len(j1)+1):
        ...
        ws2.cell(row=row_index + 3, column=8).value = j3[row_index - 1]["price"]
        ws2.cell(row=row_index + 3, column=9).value = f"=G{row_index + 3}*H{row_index + 3}"

    # ws2 테두리 설정
    for row in ws2["A2":f"I{len(j1)+3}"]:
        for cell in row:
            cell.border = thin_border

    # ws2 왼쪽 정렬
    for row in ws2["A3":f"I{len(j1)+3}"]:
        for cell in row:
