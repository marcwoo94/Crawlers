...
class Ui_Yes24_Cart(object):
    def setupUi(self, Yes24_Cart):
        Yes24_Cart.setObjectName("Yes24_Cart")
        Yes24_Cart.resize(280, 300)
        self.label = QtWidgets.QLabel(Yes24_Cart)
        self.label.setGeometry(QtCore.QRect(40, 40, 92, 24))
        font = QtGui.QFont()
        ...
        self.yes24_id = QtWidgets.QLineEdit(Yes24_Cart)
        self.yes24_id.setGeometry(QtCore.QRect(40, 70, 200, 30))
        self.yes24_id.setObjectName("yes24_id")
        self.label_2 = QtWidgets.QLabel(Yes24_Cart)
        self.label_2.setGeometry(QtCore.QRect(40, 130, 107, 24))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Bold")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.yes24_pw = QtWidgets.QLineEdit(Yes24_Cart)
        self.yes24_pw.setGeometry(QtCore.QRect(40, 160, 200, 30))
        self.yes24_pw.setObjectName("yes24_pw")
        self.pushButton = QtWidgets.QPushButton(Yes24_Cart)
        self.pushButton.setGeometry(QtCore.QRect(40, 220, 200, 40))
        font = QtGui.QFont()
        font.setFamily("Noto Sans KR Bold")
        font.setPointSize(12)
        font.setBold(True)
        ...
        self.retranslateUi(Yes24_Cart)
        QtCore.QMetaObject.connectSlotsByName(Yes24_Cart)

        self.yes24_pw.setEchoMode(QLineEdit.Password)
        self.pushButton.clicked.connect(self.scrape_yes24_cart)

    def retranslateUi(self, Yes24_Cart):
        _translate = QtCore.QCoreApplication.translate
        Yes24_Cart.setWindowTitle(_translate("Yes24_Cart", "Yes24_Cart"))
        self.label.setText(_translate("Yes24_Cart", "Yes24 아이디"))
        self.label_2.setText(_translate("Yes24_Cart", "Yes24 비밀번호"))
    ...
    def scrape_yes24_cart(self):
        ...
        driver.find_element_by_name("SMemberID").send_keys(self.yes24_id.text())
        driver.find_element_by_name("SMemberPassword").send_keys(self.yes24_pw.text())
        ...
        link_list = [book.find("a", class_="pd_a")["href"] for book in books]
        ...
        def get_book_info(url):
            ...
            try:
                ...
                title = ""
            try:
                isbn = bs_obj.find_all("td", class_="txt lastCol")[2].text.strip()  # ISBN
            except:
                isbn = ""
            try:
                publisher = gd_pub.find("a").text  # 출판사
            except:
                publisher = ""
            try:
               date = bs_obj.find_all("td", class_="txt lastCol")[0].text.replace("년 ", "/").replace("월 ", "/").replace("일", "")  # 출판일
            except:
               date = ""
            ...
            try:
                author = contributors.find_all("a")[0].text  # 저자
            except:
                try:
                    author = contributors.text.strip()
                except:
                    author = ""
            try:
                translator = contributors.find_all("a")[1].text  # 역자
            except:
                translator = ""
            try:
                remarks = bs_obj.find("span", class_="gd_feature").text.split("[")[1].split("]")[0].strip()  # 표지
            except:
                remarks = ""
            try:
                category = category_list[0].find_all("a")[1].text + ", " + category_list[1].find_all("a")[1].text + ", " + category_list[2].find_all("a")[1].text  # 카테고리
            except:
                try:
                    category = category_list[0].find_all("a")[1].text + ", " + category_list[1].find_all("a")[1].text
                except:
                    category = category_list[0].find_all("a")[1].text
        ...
        book_info = [get_book_info(url) for url in link_list]
...
if __name__ == "__main__":
    ...
    Yes24_Cart = QtWidgets.QDialog()
    ui = Ui_Yes24_Cart()
    ui.setupUi(Yes24_Cart)
    Yes24_Cart.show()
    sys.exit(app.exec_())
